...
// test runs. Manager.saveConfig and SkillsManager.saveConfig persist the
// full config to config.ConfigFile's location, so a test that forgets its
// own t.Setenv isolation must corrupt this temp root — never the
...
func TestMain(m *testing.M) {
	root, err := os.MkdirTemp("", "squad-source-test-xdg-*")
	if err != nil {
